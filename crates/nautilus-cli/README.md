...
- User-facing commands perform a best-effort GitHub release check before running and cache the result for 24 hours in the local Nautilus data directory. The internal `engine serve` command is excluded so JSON-RPC clients never receive extra output or startup latency. Use the global `--no-update-check` flag or `NAUTILUS_NO_UPDATE_CHECK=1` to disable the check.
