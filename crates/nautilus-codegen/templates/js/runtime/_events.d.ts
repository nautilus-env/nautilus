...
export type CrudOperation = 'create' | 'createMany' | 'update' | 'updateMany' | 'delete' | 'deleteMany';
...
export interface CrudEventContext<TModel = ModelEventToken, TOperation extends CrudOperation = CrudOperation, TArgs extends object = Record<string, unknown>, TPayload extends object = Record<string, unknown>, TResult = unknown, TError = unknown, TState extends object = Record<string, unknown>> {
    readonly db: unknown;
...
export type CrudEventHandler<TContext extends CrudEventContext = CrudEventContext> = (context: TContext) => unknown | Promise<unknown>;
export interface ModelEventRegistrar<TContext extends CrudEventContext = CrudEventContext> {
    ...
    (handler: CrudEventHandler<TContext>, options: EventPriorityOptions): CrudEventHandler<TContext>;
    (phase: EventPhaseValue, options?: EventPriorityOptions): (handler: CrudEventHandler<TContext>) => CrudEventHandler<TContext>;
    (options: EventRegistrationOptions): (handler: CrudEventHandler<TContext>) => CrudEventHandler<TContext>;
}
...
export declare class StopPropagation extends Error {
    ...
    constructor(options?: StopPropagationOptions | undefined);
}
export declare function createModelEvents<TContexts extends ModelEventContexts = ModelEventContexts>(modelName: string): ModelEventToken<TContexts>;
...
export declare function runCrudEvent(context: CrudEventContext, options?: {
    handleStopPropagation?: boolean;
} | undefined): Promise<StopPropagation | null>;
export declare function defaultCrudResult(operation: CrudOperation, returnData?: boolean): unknown;
export declare function resolveStopResult(stop: StopPropagation, defaultResult: unknown): unknown;
