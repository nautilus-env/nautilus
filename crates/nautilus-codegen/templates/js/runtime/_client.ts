...
import type { JsonRpcRequest, JsonRpcResponse } from './_protocol.js';
...
export class NautilusClient {
  declare protected readonly engine: EngineProcess;
  /** @internal */
  declare private nextId: number;
  /** @internal */
  declare private pending: Map<number, PendingRequest>;
  /** @internal */
  declare private partialData: Map<number, unknown[]>;
  /** @internal */
  declare private streams: Map<number, PendingStreamRequest>;
  /** @internal */
  declare private rl: readline.Interface | null;
  declare _delegates: Record<string, unknown>;
  /** @internal */
  declare private schemaPath: string;
  ...
  private async _handshake(): Promise<void> {
    ...
    const v = (response as Record<string, unknown> | undefined)?.['protocolVersion'];
  ...
  protected async _startTransaction(timeoutMs = 5000, isolationLevel?: IsolationLevel): Promise<string> {
  ...
  protected async _runTransactionBatch(
    ...
    const result = await this._rpc('transaction.batch', params) as Record<string, unknown> | undefined;
    return Array.isArray(result?.['results']) ? result['results'] : [];
  }

  async $metrics(options: { reset?: boolean } | undefined = undefined): Promise<EngineMetrics> {
