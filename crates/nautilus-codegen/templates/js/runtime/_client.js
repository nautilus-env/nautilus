...
export class NautilusClient {
    constructor(schemaPath, options) {
        ...
        this.engine = new EngineProcess(undefined, options?.migrate ?? false, options?.pool);
        this.nextId = 0;
    ...
    _startReading() {
        ...
        this.rl.on('line', (line) => {
            ...
            if (stream) {
                if (response.error) {
                    this._finishStream(id, errorFromCode(response.error.code, response.error.message, response.error.data));
                }
                else {
            ...
            if (response.partial === true) {
                ...
                if (response.error) {
                    this.partialData.delete(id);
                    pending.reject(errorFromCode(response.error.code, response.error.message, response.error.data));
                }
                else {
    ...
    async _handshake() {
        ...
        if (v !== PROTOCOL_VERSION) {
            await this.disconnect();
            throw new HandshakeError(`Protocol version mismatch: engine uses ${v}, client expects ${PROTOCOL_VERSION}`);
        }
    }
    ...
    async _runTransactionBatch(operations, options) {
        ...
        if (options?.isolationLevel != null)
            params['isolationLevel'] = options.isolationLevel;
    ...
    async _runTransactionCallback(fn, options) {
        const txId = await this._startTransaction(options?.timeout ?? 5000, options?.isolationLevel);
