...
export function defaultCrudResult(operation, returnData = true) {
    ...
    if (operation === 'createMany')
        return [];
    if (operation === 'update' || operation === 'deleteMany')
        return returnData ? [] : 0;
...
function modelEventRegistrar(modelName, operation) {
    return function registerOrDecorate(phaseOrHandler = EventPhase.Before, options = undefined) {
        if (typeof phaseOrHandler === 'function') {
            registerEventHandler(modelName, operation, EventPhase.Before, phaseOrHandler, eventPriority(options));
