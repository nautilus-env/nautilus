...
export function errorFromCode(code, message, data) {
    ...
    if (code === 3002)
        return new ConstraintViolationError(`[${code}] ${message}`, details);
    if (code === 3003)
        return new QueryTimeoutError(`[${code}] ${message}`, details);
    if (code === 3004)
        return new NotFoundError(`[${code}] ${message}`, details);
    if (code === 3005)
        return new UniqueConstraintError(`[${code}] ${message}`, details);
    if (code === 3006)
        return new ForeignKeyConstraintError(`[${code}] ${message}`, details);
    if (code === 3007)
        return new CheckConstraintError(`[${code}] ${message}`, details);
    if (code === 3008)
        return new NullConstraintError(`[${code}] ${message}`, details);
    if (code === 3009)
        return new DeadlockError(`[${code}] ${message}`, details);
    if (code === 3010)
        return new SerializationError(`[${code}] ${message}`, details);
    if (code >= 3000 && code < 4000)
        return new DatabaseError(`[${code}] ${message}`, details);
    if (code === 4002)
        return new TransactionTimeoutError(`[${code}] ${message}`, details);
    if (code >= 4001 && code <= 4004)
        return new TransactionError(`[${code}] ${message}`, details);
    if (code >= 9000 && code < 10000)
        return new InternalError(`[${code}] ${message}`, details);
