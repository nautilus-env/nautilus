...
impl SchemaInspector {
    pub(super) async fn inspect_mysql(&self) -> Result<LiveSchema> {
        ...
        let view_rows = sqlx::query(
            "SELECT CAST(table_name AS CHAR) AS table_name              FROM information_schema.tables              WHERE table_schema = DATABASE()                AND table_type   = 'VIEW'              ORDER BY table_name",
        )
        .fetch_all(&pool)
        .await
        .map_err(|e| MigrationError::Database(e.to_string()))?;
        ...
        let view_names: Vec<String> = view_rows
            ...
            .map_err(|e| MigrationError::Database(e.to_string()))?
            .into_iter()
            .filter(|view_name| !view_name.starts_with("_nautilus_"))
            .collect();
        ...
        for table_name in table_names {
            ...
            let mut columns = build_mysql_columns(&col_rows)?;
        ...
        for view_name in view_names {
            let col_rows = columns_by_table.remove(&view_name).unwrap_or_default();
            let mut columns = build_mysql_columns(&col_rows)?;
...
fn build_mysql_columns(rows: &[sqlx::mysql::MySqlRow]) -> Result<Vec<LiveColumn>> {
    use sqlx::Row as _;
